...
(()=>{const segments=location.pathname.split('/').filter(Boolean);const segment=segments.find(value=>['en','my','ms','zh','cn'].includes(value.toLowerCase()))?.toLowerCase();const label=document.querySelector('[data-loader-label]');if(label)label.textContent=segment==='my'||segment==='ms'?'Memuatkan halaman…':segment==='zh'||segment==='cn'?'正在加载页面…':'Loading page…';const finish=()=>requestAnimationFrame(()=>{const loader=document.querySelector('#page-transition-loader');if(!loader)return;loader.hidden=true;loader.removeAttribute('data-visible')});const duelPage=location.pathname.endsWith('/multiplayer/')||location.pathname.endsWith('/multiplayer')||location.pathname.endsWith('/multi-duel/')||location.pathname.endsWith('/multi-duel');const retryKey='vocabduel-duel-asset-retry:'+location.pathname;const recover=()=>{if(!duelPage)return;let retried=false;try{retried=sessionStorage.getItem(retryKey)==='1'}catch{}if(retried){const root=document.querySelector('#multiplayer-app, #multi-duel-app');if(root){const copy=segment==='my'||segment==='ms'?['Duel tidak dapat dimuatkan','Muat semula halaman untuk mencuba lagi.','Muat semula']:segment==='zh'||segment==='cn'?['无法加载对战','请刷新页面后重试。','刷新页面']:['Duel could not load','Refresh the page to try again.','Refresh page'];root.innerHTML='<section class="panel unavailable-panel" role="alert"><h2>'+copy[0]+'</h2><p>'+copy[1]+'</p><button class="button primary" type="button">'+copy[2]+'</button></section>';root.querySelector('button')?.addEventListener('click',()=>{try{sessionStorage.removeItem(retryKey)}catch{}location.reload()})}return}try{sessionStorage.setItem(retryKey,'1')}catch{}const next=new URL(location.href);next.searchParams.set('_asset_retry',Date.now().toString());location.replace(next.href)};window.addEventListener('error',event=>{if(event.target instanceof HTMLScriptElement&&event.target.type==='module')recover()},true);window.addEventListener('load',()=>{window.setTimeout(()=>{const root=document.querySelector('#multiplayer-app, #multi-duel-app');if(root?.querySelector('.loading-panel'))recover();else try{sessionStorage.removeItem(retryKey)}catch{}},8000)},{once:true});if(document.readyState==='complete')finish();else window.addEventListener('load',finish,{once:true})})()
`

new Function(initialLoaderScript)
