...
import { localiseDuelRoot, localisedDuelDialog, localisedDuelText } from './duel-localisation'
...
const roomOperations = new Proxy({} as RoomApi, {
  get: (_target, property: string) => async (...args: unknown[]) => {
    const api = await (roomApiPromise ??= import('./firebase/rooms'))
    const operation = api[property as keyof RoomApi] as (...values: unknown[]) => unknown
    return operation(...args)
  },
})
const { beginRematch, closeRoundEarly, createRoom, finishMatch, joinRoom, leaveRoom, markPlaying, observePresence, observeServerOffset, resumeRoom, setReady, startMatch, submitAnswer, subscribeRoom, voteRematch } = roomOperations
...
function bindActions(): void {
  ...
  root.querySelector<HTMLButtonElement>('[data-share]')?.addEventListener('click', async (event) => {
    ...
    const text = language === 'ms' ? `Sertai duel perkataan VocabDuel saya! Kod bilik: ${roomCode}` : language === 'en' ? `Join my VocabDuel word duel! Room code: ${roomCode}` : `加入我的 VocabDuel 单词对战！房间码：${roomCode}`
    const title = language === 'ms' ? 'Sertai duel perkataan VocabDuel saya' : language === 'en' ? 'Join my VocabDuel word duel' : '加入我的 VocabDuel 单词对战'
    try {
...
async function initialize(): Promise<void> {
  ...
  try {
    ...
    root.innerHTML = `<section class="panel unavailable-panel" role="alert"><h2>无法连接多人对战</h2><p>${escapeHtml(localisedDuelText(error instanceof Error ? error.message : '请稍后再试。', language))}</p><button class="button primary" data-retry>重新连接</button><a class="button ghost" href="${SITE.routes.play}">单人练习</a></section>`
    localiseDuelRoot(root, language)
